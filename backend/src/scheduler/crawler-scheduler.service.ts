...
interface ToggleTargetResponse {
  success: boolean;
  message: string;
}
...
export class CrawlerSchedulerService {
  ...
  async handleScheduledCrawl() {
    ...
    try {
      ...
      const totalSaved = results.reduce((sum, result) => sum + result.savedCount, 0);
   ...
   * 手动触发爬取任务（可用于调试）
   */
  async triggerManualCrawl(targetName?: string): Promise<ManualCrawlResponse> {
    const targets = CRAWLER_TARGETS.filter(target => target.enabled);
    ...
    const target = targetName ? targets.find(t => t.name === targetName) : targets[0];
   ...
   * 测试连接
   */
  async testConnection(targetName?: string): Promise<ConnectionTestResponse> {
    const targets = CRAWLER_TARGETS.filter(target => target.enabled);
   ...
   * 启用/禁用爬虫目标
   */
  toggleTarget(targetName: string, enabled: boolean): ToggleTargetResponse {
