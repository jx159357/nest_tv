...
- Follow-up torrent category-search enhancement completed: torrent search now applies category filtering on the backend instead of trimming one page of results in the browser, query params are validated/normalized through dedicated DTOs, and `TorrentView` now refreshes search + ranking shelves when the category changes so paginated category searches stop dropping valid matches.
- Follow-up torrent deep-link persistence enhancement completed: `TorrentView` now hydrates `keyword` / `category` / `hash` from the browser URL, writes those values back while the workspace state changes, and can restore both the current search slice and the selected magnet detail after refresh or copied links.
- Follow-up download-to-torrent roundtrip enhancement completed: magnet tasks in `DownloadsView` now expose a direct “查看磁力详情” link that jumps back into `TorrentView` with the task’s `hash` and filename prefilled, so queued magnet work can return to the exact detail workspace instead of only copying the raw URL.
- Follow-up magnet dedupe enhancement completed: the downloads store now treats magnet tasks with the same `infoHash` as the same logical job even when tracker lists or URI ordering differ, so repeated queue actions update the existing task instead of generating duplicate magnet records.
- Follow-up remote magnet dedupe enhancement completed: download-task hydration now also collapses remote magnet records by `infoHash`, preferring the newest task snapshot across different `clientId`s, so older duplicate magnet history entries no longer reappear in the user workspace after sync.
- Follow-up backend magnet dedupe enhancement completed: download-task upserts now also try to match an existing user magnet record by `infoHash` before inserting a new row, so changing `clientId` on the frontend no longer creates duplicate magnet history rows in the backend database.
- Follow-up backend magnet cleanup enhancement completed: when a magnet upsert hits an existing `infoHash`, the backend now also removes older duplicate rows for that same user/hash after saving the keeper snapshot, so historical duplicate magnet records get folded back into one row instead of lingering forever.
- Follow-up user download API dedupe enhancement completed: `/download-tasks/user/me` and the matching stats endpoint now also collapse duplicate magnet rows by `infoHash`, so user-side paging and summary cards no longer count stale duplicate magnet history even before every old row has been rewritten.
- Follow-up admin download dedupe enhancement completed: backend admin download-task listing and system download-task counters now also collapse duplicate magnet rows by `infoHash`, so the admin workspace/dashboard stops double-counting stale duplicate magnet history while older rows are still being cleaned up.
- Follow-up magnet delete cleanup enhancement completed: deleting a visible user magnet task now also removes any older duplicate rows for the same `infoHash`, so a stale duplicate no longer reappears in the user downloads workspace after the current task is deleted.
- Follow-up magnet clear cleanup enhancement completed: clearing completed or failed download tasks now removes the whole visible magnet group keyed by `infoHash`, so an older duplicate row in another status no longer resurfaces after the current task batch is cleaned.
- Follow-up torrent-to-download exact-hash enhancement completed: `TorrentView` now deep-links into `DownloadsView` with both the human keyword and the exact magnet `hash`, while `DownloadsView` can hydrate and clear that hash lock from the URL so task handoff lands on the exact magnet job instead of relying only on fuzzy title matching.
- Follow-up backend magnet exact-match enhancement completed: service-side magnet dedupe/delete lookup now filters user magnet tasks by parsed `infoHash` instead of URL substring matching, avoiding accidental collisions from partial `LIKE` hits while keeping the same dedupe behavior.
- Follow-up admin hash-filter enhancement completed: admin download-task queries now support an exact magnet `hash` filter end to end, `AdminDownloadTasksView` persists that hash in route chips/query state, and magnet task recommendations can jump directly into a same-hash admin workspace instead of relying only on fuzzy search text.
- Follow-up admin hash-log enhancement completed: download-task admin logs now also capture magnet `infoHash`, `AdminLogsView` supports exact hash filtering in route/query state, and log/detail links can jump back into same-hash admin download workspaces instead of being limited to `clientId` and task ID only.
- Follow-up admin log deep-link fix completed: `AdminDownloadTasksView` now sends the correct `downloadTaskId` query key when jumping into `AdminLogsView`, and magnet task log links also carry the same exact `hash`, so task-to-log drill-down lands on the intended log slice instead of dropping the task filter.
- Follow-up admin hash-toolbar enhancement completed: `AdminDownloadTasksView` now exposes an explicit `hash` input in the main filter bar, so operators can run exact magnet task queries directly from the page instead of only arriving through route state or recommendation shortcuts.
- Follow-up admin task-id exact-filter enhancement completed: admin download-task queries now also accept an exact `taskId`, and `AdminDownloadTasksView` passes the routed detail task id through to the backend list API, so task detail deep links no longer rely only on fuzzy `search` hits to keep the intended row visible.
- Follow-up admin task-id toolbar enhancement completed: `AdminDownloadTasksView` now exposes an explicit `taskId` input and filter chip in the main toolbar, making the exact task lock visible and directly editable instead of living only in route state or detail toggles.
- Follow-up remote multi-page hydration enhancement completed: the frontend downloads store now pulls remote download tasks page by page instead of stopping after the first `limit=200` response, so larger user histories no longer silently drop older synced tasks from the downloads workspace.
- Follow-up admin action cleanup enhancement completed: retry/cancel actions in `AdminService` now also remove older duplicate magnet rows for the same user/hash after saving the visible task snapshot, so admin interventions actively collapse stale duplicate records instead of only hiding them at read time.
- Follow-up admin stale-task fallback enhancement completed: log-to-task deep links now prefer the exact `hash` workspace when available, and `AdminDownloadTasksView` automatically drops an invalid routed `taskId` and reloads with the remaining filters, so cleaned-up duplicate rows no longer strand operators on an empty task page.
- Follow-up admin log-id persistence enhancement completed: `AdminLogsView` now syncs the opened log detail id into route/query state, restores the same expanded log after refresh/share, and clears a stale `logId` automatically when that log no longer exists in the current result set.
- Follow-up downloads hash-toolbar enhancement completed: `DownloadsView` now exposes an explicit magnet `hash` input beside the keyword filter, so exact magnet task queries can be typed directly instead of only arriving through route state or deep links.
- Follow-up admin log-id toolbar enhancement completed: `AdminLogsView` now exposes an explicit `logId` input in the main filter bar, so operators can jump to an exact log row directly instead of only arriving through deep links or existing route state.
- Follow-up admin log-to-magnet narrowing enhancement completed: when a log entry carries magnet `infoHash`, `AdminLogsView` now routes back into `AdminDownloadTasksView` with both `hash` and `type=magnet`, reducing unrelated rows when operators pivot from logs back into the task workspace.
- Follow-up admin legacy-log fallback enhancement completed: `AdminLogsView` can now still jump back into `AdminDownloadTasksView` when an older log only carries `downloadTaskId` without `clientId` or `hash`, using the exact task-id filter instead of dropping the task link entirely.
- Follow-up hash-input normalization enhancement completed: `DownloadsView`, `AdminDownloadTasksView`, and `AdminLogsView` now normalize typed magnet hashes to trimmed lowercase at input time, so exact-hash filters stop failing on pasted uppercase or spaced values and route/query state stays canonical.
- Follow-up admin exact-clientId enhancement completed: admin download-task queries now support an explicit `clientId` filter, `AdminDownloadTasksView` exposes it in the main toolbar/chips, and `AdminLogsView` now routes legacy log entries back with exact `clientId` filtering instead of fuzzy search.
