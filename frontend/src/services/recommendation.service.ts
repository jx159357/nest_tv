...
export const useRecommendationService = () => {
  ...
  const calculateRecommendationScore = (
    ...
    if (media.rating && userPreferences.avgRatingByType[media.type]) {
      const ratingDiff = Math.abs(
        media.rating - userPreferences.avgRatingByType[media.type]
      )
    ...
    switch (type) {
      case 'personalized':
        ...
        if (media.genres && media.genres.some(g => userPreferences.preferredGenres.includes(g))) {
          reasons.push(`包含您偏好的${media.genres.find(g => userPreferences.preferredGenres.includes(g))}类型`)
        }
  ...
  const mlRecommendation = async (limit: number = 10) => {
    ...
    try {
      ...
      const userProfile = {
        ...
        avgWatchTime: 0,
        completionRate: 0,
        ratingTrend: 'increasing' | 'stable' | 'decreasing' as const
      }
      ...
      userHistory.forEach((item: any) => {
        ...
        if (item.mediaResource?.genres) {
          item.mediaResource.genres.forEach((genre: string) => {
            userProfile.watchedGenres[genre] = 
              (userProfile.watchedGenres[genre] || 0) + 1
          })
        }
      ...
      userProfile.avgWatchTime = userHistory.reduce(
        (sum: number, item: any) => sum + (item.currentTime || 0), 0
      ) / userHistory.length
  ...
  const calculateMLScore = (media: MediaResource, userProfile: any) => {
    ...
    score += (typeScore / Math.max(1, Object.values(userProfile.watchedTypes).reduce((a, b) => a + b, 0))) * 30

    // 类型权重 (25%)
    if (media.genres) {
      const genreScore = media.genres.reduce((sum, genre) => 
        sum + (userProfile.watchedGenres[genre] || 0), 0
      )
