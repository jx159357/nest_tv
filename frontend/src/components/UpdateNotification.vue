...
<style scoped>
.update-slide-enter-active, .update-slide-leave-active { transition: all 0.35s cubic-bezier(0.4, 0, 0.2, 1); }
...
.update-icon {
  ...
  color: #a5b4fc; animation: spin 2s linear infinite;
}
.update-icon svg { width: 20px; height: 20px; }
@keyframes spin { to { transform: rotate(360deg); } }

.update-text { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 2px; }
.update-title { font-size: 14px; font-weight: 600; color: #e2e8f0; }
.update-desc { font-size: 12px; color: #94a3b8; }
...
.update-btn--now:hover { background: #4f46e5; }
.update-btn--later {
  background: rgba(255, 255, 255, 0.08); color: #94a3b8; border: 1px solid rgba(255, 255, 255, 0.1);
}
.update-btn--later:hover { background: rgba(255, 255, 255, 0.12); }

@media (max-width: 640px) {
  .update-content { flex-wrap: wrap; }
