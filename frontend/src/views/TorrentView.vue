      ...
      <section class="mb-8 rounded-2xl bg-white p-6 shadow-sm">
        <div class="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.3fr)_minmax(320px,0.7fr)]">
          <div>
            <div class="mb-4 flex items-center justify-between gap-3">
              <div>
                ...
                <p class="mt-1 text-sm text-gray-600">按标题、来源名或媒体描述搜索已入库的磁力资源。</p>
              </div>
            </div>

            <form class="grid grid-cols-1 gap-3 md:grid-cols-[minmax(0,1fr)_180px_auto]" @submit.prevent="searchTorrents(1)">
              <input
                v-model="keyword"
                type="text"
            ...
            <div v-if="searchError" class="mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600">
              {{ searchError }}
            </div>
            ...
            <div class="mt-5 overflow-hidden rounded-xl border border-slate-200 bg-white">
              ...
              <div v-if="searchLoading" class="px-4 py-10 text-center text-sm text-slate-500">加载中...</div>
              <div v-else-if="searchResults.length === 0" class="px-4 py-10 text-center text-sm text-slate-500">
                  ...
                  <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div class="min-w-0">
                      ...
                      <div class="mt-1 text-xs text-slate-500">
                        {{ item.mediaTitle || `媒体 #${item.mediaResourceId}` }} · {{ item.category || '未分类' }}
                      </div>
                    </div>
                    <div class="flex shrink-0 flex-wrap gap-2 text-xs text-slate-500">
                      <span class="rounded-full bg-slate-100 px-2.5 py-1">{{ formatSize(item.size) }}</span>
                      <span class="rounded-full bg-slate-100 px-2.5 py-1">做种 {{ item.seeders ?? '—' }}</span>
                      <span class="rounded-full bg-slate-100 px-2.5 py-1">吸血 {{ item.leechers ?? '—' }}</span>
                    </div>
                  </div>
            ...
            <div v-if="searchPagination.totalPages > 1" class="mt-4 flex items-center justify-between text-sm text-slate-600">
              <span>第 {{ searchPagination.page }} / {{ searchPagination.totalPages }} 页，共 {{ searchPagination.total }} 条</span>
          ...
          <div class="rounded-2xl border border-slate-200 bg-slate-50 p-5">
            ...
            <p class="mt-1 text-sm text-gray-600">粘贴 magnet 链接，快速查看解析出的 infoHash、tracker 和 web seed。</p>
            ...
            <div v-if="parseError" class="mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600">
              {{ parseError }}
            </div>

            <div v-if="parsedMagnet" class="mt-4 space-y-3 rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-700">
              <div><span class="font-medium text-slate-900">名称：</span>{{ parsedMagnet.name || '未命名' }}</div>
              <div><span class="font-medium text-slate-900">InfoHash：</span>{{ parsedMagnet.infoHash }}</div>
              <div><span class="font-medium text-slate-900">Tracker：</span>{{ parsedMagnet.announce.length }}</div>
              <div><span class="font-medium text-slate-900">Web Seed：</span>{{ parsedMagnet.urlList.length }}</div>
              <div><span class="font-medium text-slate-900">关键字：</span>{{ parsedMagnet.keywords.join(' / ') || '无' }}</div>
            </div>
          </div>
        </div>
      </section>

      <section class="grid grid-cols-1 gap-8 xl:grid-cols-[minmax(0,0.95fr)_minmax(360px,1.05fr)_minmax(360px,1fr)]">
        <div class="rounded-2xl bg-white p-6 shadow-sm">
          <div class="mb-4 flex items-center justify-between">
            ...
            <button class="text-sm text-blue-600 hover:underline" @click="loadPopularTorrents">刷新</button>
          </div>
          ...
          <div v-else class="space-y-3">
            ...
            >
              ...
              <div class="mt-1 text-xs text-slate-500">做种 {{ item.seeders ?? '—' }} · {{ formatSize(item.size) }}</div>
            </button>
          </div>
        </div>
        ...
        <div class="rounded-2xl bg-white p-6 shadow-sm">
          <div class="mb-4 flex items-center justify-between">
            ...
            <button class="text-sm text-blue-600 hover:underline" @click="loadLatestTorrents">刷新</button>
          </div>
          ...
          <div v-else class="space-y-3">
            ...
            >
              <div class="font-medium text-slate-900">{{ item.name }}</div>
              <div class="mt-1 text-xs text-slate-500">{{ formatDateTime(item.added) }} · {{ item.category || '未分类' }}</div>
            </button>
          </div>
        </div>
        ...
        <div class="rounded-2xl bg-white p-6 shadow-sm">
          ...
          <div v-if="selectedLoading" class="py-10 text-center text-sm text-slate-500">加载详情中...</div>
          <div v-else-if="selectedError" class="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600">
            {{ selectedError }}
          </div>
          <div v-else-if="selectedInfo" class="space-y-4 text-sm text-slate-700">
            <div>
              <div class="text-lg font-semibold text-slate-900">{{ selectedInfo.name || selectedInfo.infoHash }}</div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class="rounded-xl bg-slate-50 p-3">
                ...
                <div class="mt-1 text-lg font-semibold text-slate-900">{{ selectedInfo.relatedSourcesCount }}</div>
              </div>
              <div class="rounded-xl bg-slate-50 p-3">
                <div class="text-xs text-slate-500">体积</div>
                <div class="mt-1 text-lg font-semibold text-slate-900">{{ formatSize(selectedInfo.size) }}</div>
              </div>
            </div>
              ...
              <div class="flex flex-wrap gap-2 text-xs">
                <span
                  :class="[
                    ...
                    selectedHealth.isHealthy ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700',
                  ]"
                >
                  {{ selectedHealth.isHealthy ? '可用' : '异常' }}
                </span>
                <span class="rounded-full bg-slate-200 px-2.5 py-1 text-slate-700">做种 {{ selectedHealth.seeders ?? '—' }}</span>
                <span class="rounded-full bg-slate-200 px-2.5 py-1 text-slate-700">吸血 {{ selectedHealth.leechers ?? '—' }}</span>
              </div>
              <div class="mt-2 text-xs text-slate-500">最近校验 {{ formatDateTime(selectedHealth.lastChecked) }}</div>
            </div>

            <div>
              <div class="mb-2 text-sm font-medium text-slate-900">关联媒体</div>
              <div v-if="selectedInfo.linkedMedia.length === 0" class="text-xs text-slate-500">暂无关联媒体</div>
            ...
            <details class="rounded-xl border border-slate-200 bg-slate-50 p-4">
              <summary class="cursor-pointer text-sm font-medium text-slate-900">文件列表（{{ selectedInfo.files.length }}）</summary>
              <div class="mt-3 space-y-2 text-xs text-slate-600">
                <div v-for="file in selectedInfo.files" :key="file.name" class="rounded-lg bg-white px-3 py-2">
                  {{ file.name }}<span v-if="file.size"> · {{ formatSize(file.size) }}</span>
                </div>
              </div>
            </details>
          </div>
          <div v-else class="py-10 text-center text-sm text-slate-500">从左侧搜索结果或榜单中选择一个磁力资源。</div>
        </div>
      </section>
    </div>
  </div>
...
<script setup lang="ts">
  ...
  const loadPopularTorrents = async () => {
    ...
    try {
      popularTorrents.value = await torrentApi.getPopular({ limit: 8, category: category.value || undefined });
    } catch {
  ...
  const loadLatestTorrents = async () => {
    ...
    try {
      latestTorrents.value = await torrentApi.getLatest({ limit: 8, category: category.value || undefined });
    } catch {
  ...
  const selectTorrent = async (hash: string) => {
    ...
    try {
      const [info, health] = await Promise.all([torrentApi.getInfo(hash), torrentApi.getHealth(hash)]);
