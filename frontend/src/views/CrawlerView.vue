  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
        ...
        <div v-else-if="crawlerTargets.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div
          ...
          >
            <div class="flex justify-between items-start">
              ...
              >
                {{ target.status === 'online' ? '在线' : target.status === 'offline' ? '离线' : '未知' }}
              </span>
            </div>
      ...
      <div v-if="crawlResult" class="bg-white rounded-lg shadow-sm p-6 mb-6">
        ...
        <div v-if="crawlResult.success" class="space-y-4">
          ...
          <div class="border border-gray-200 rounded-lg p-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p><span class="font-medium">导演:</span> {{ crawlResult.data.director || '无' }}</p>
                <p><span class="font-medium">主演:</span> {{ crawlResult.data.actors || '无' }}</p>
              </div>
              <div>
                <p><span class="font-medium">来源:</span> {{ crawlResult.data.source }}</p>
                <p><span class="font-medium">上映日期:</span> {{ crawlResult.data.releaseDate || '无' }}</p>
                <p><span class="font-medium">剧集数:</span> {{ crawlResult.data.episodeCount || '无' }}</p>
                <p v-if="crawlResult.data.poster"><span class="font-medium">海报:</span> {{ crawlResult.data.poster }}</p>
              </div>
            </div>
          ...
          <div v-if="quickCrawlForm.saveToDatabase && crawlResult.data.id" class="p-4 bg-blue-50 rounded-lg">
      ...
      <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <form @submit.prevent="batchCrawl" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">选择爬虫目标 *</label>
            <select
          ...
          <div v-if="batchCrawlResult.data && batchCrawlResult.data.crawledData" class="mt-3">
            ...
            <div class="space-y-2 max-h-60 overflow-y-auto">
              ...
              >
                ...
                <p class="text-gray-600 text-sm">评分: {{ data.rating }} | 来源: {{ data.source }}</p>
              </div>
            </div>
          </div>
...
const testConnection = async (targetName) => {
  try {
    ...
    const response = await authStore.api.get(`/crawler/test-connection?targetName=${encodeURIComponent(targetName)}`)
