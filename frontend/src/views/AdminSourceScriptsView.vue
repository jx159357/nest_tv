<template>
  <div class="space-y-6">
    ...
    <div v-else-if="scripts.length === 0" class="rounded-xl border border-white/[0.06] bg-white/[0.03] py-16 text-center">
      ...
      >
        <div class="min-w-0 flex-1">
          <div class="flex items-center gap-3">
            ...
            <code class="rounded bg-slate-800 px-2 py-0.5 text-xs text-slate-400">{{ script.key }}</code>
            <span
              class="rounded-full px-2 py-0.5 text-xs font-medium"
              :class="script.enabled ? 'bg-emerald-500/15 text-emerald-400' : 'bg-slate-500/15 text-slate-400'"
            >
              {{ script.enabled ? '已启用' : '已禁用' }}
            </span>
          </div>
          <p v-if="script.description" class="mt-1 truncate text-xs text-slate-500">{{ script.description }}</p>
    ...
    <Transition name="modal">
      ...
      >
        <div class="flex h-[90vh] w-[90vw] max-w-5xl flex-col rounded-2xl border border-white/10 bg-[#0f1520] shadow-2xl">
          ...
          <div class="flex flex-1 overflow-hidden">
            <div class="flex w-80 flex-col border-r border-white/[0.06] p-4">
              ...
              <div class="mt-4 flex-1 overflow-auto rounded-lg border border-white/[0.06] bg-slate-900/50 p-3">
                <h3 class="mb-2 text-xs font-medium text-slate-400">测试结果</h3>
                <div v-if="testResult">
                  <div class="flex items-center gap-2">
                    <span
                      class="rounded-full px-2 py-0.5 text-xs"
                      :class="testResult.ok ? 'bg-emerald-500/15 text-emerald-400' : 'bg-red-500/15 text-red-400'"
                    >
                      {{ testResult.ok ? '成功' : '失败' }}
                    </span>
                    <span class="text-xs text-slate-500">{{ testResult.duration }}ms</span>
                  </div>
                  <pre v-if="testResult.error" class="mt-2 whitespace-pre-wrap break-all text-xs text-red-300">{{ testResult.error }}</pre>
                  <pre v-if="testResult.logs?.length" class="mt-2 whitespace-pre-wrap break-all text-xs text-slate-400">{{ testResult.logs.join('\n') }}</pre>
                  <pre v-if="testResult.result" class="mt-2 max-h-60 overflow-auto whitespace-pre-wrap break-all text-xs text-slate-300">{{ JSON.stringify(testResult.result, null, 2) }}</pre>
                </div>
