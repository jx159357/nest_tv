<template>
  <div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div>
        <h1 class="text-2xl font-bold text-slate-100">角色权限管理</h1>
        <p class="mt-2 text-slate-400">维护后台角色、权限字典，以及角色与权限码之间的分配关系。</p>
      </div>
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
      <div class="rounded-xl border border-white/[0.06] bg-white/[0.03] p-5 shadow-xl">
        <div class="text-sm text-slate-400">角色总数</div>
        <div class="mt-3 text-3xl font-semibold text-slate-100">{{ roles.length }}</div>
        <div class="mt-2 text-xs text-slate-500">
          启用 {{ activeRoleCount }} / 停用 {{ inactiveRoleCount }}
        </div>
      </div>
      <div class="rounded-xl border border-white/[0.06] bg-white/[0.03] p-5 shadow-xl">
        <div class="text-sm text-slate-400">权限总数</div>
        <div class="mt-3 text-3xl font-semibold text-slate-100">{{ permissions.length }}</div>
        <div class="mt-2 text-xs text-slate-500">
          启用 {{ activePermissionCount }} / 停用 {{ inactivePermissionCount }}
        </div>
      </div>
      <div class="rounded-xl border border-white/[0.06] bg-white/[0.03] p-5 shadow-xl">
        <div class="text-sm text-slate-400">角色权限映射</div>
        <div class="mt-3 text-3xl font-semibold text-slate-100">{{ assignedPermissionCount }}</div>
        <div class="mt-2 text-xs text-slate-500">已分配到角色的权限码数量</div>
      </div>
      <div class="rounded-xl border border-white/[0.06] bg-white/[0.03] p-5 shadow-xl">
        <div class="text-sm text-slate-400">当前视图</div>
        <div class="mt-3 text-xl font-semibold text-slate-100">
    ...
    <div class="rounded-xl border border-white/[0.06] bg-white/[0.03] shadow-xl">
      <div class="border-b border-white/[0.06] px-6 pt-4">
      ...
      <div v-if="activeTab === 'roles'" class="p-6">
        <div v-if="rolesLoading" class="py-16 text-center text-sm text-slate-400">
          角色列表加载中...
        </div>
        <div v-else-if="roles.length === 0" class="py-16 text-center text-sm text-slate-400">
          暂无角色数据，先创建一个角色吧。
        </div>
          ...
          <table class="min-w-full divide-y divide-white/[0.06]">
            ...
            <tbody class="divide-y divide-white/[0.06]">
              <tr v-for="role in roles" :key="role.id">
                ...
                <td class="px-4 py-4 text-sm text-slate-400">
                  ...
                  <div class="mt-2 flex flex-wrap gap-2">
                    <span
                      ...
                      :key="`${role.id}-${code}`"
                      class="rounded-full bg-slate-500/15 px-2.5 py-1 text-xs text-slate-300"
                    >
                ...
                <td class="px-4 py-4 text-sm text-slate-400">{{ formatDate(role.updatedAt) }}</td>
      ...
      <div v-else class="p-6">
        <div v-if="permissionsLoading" class="py-16 text-center text-sm text-slate-400">
          权限列表加载中...
        </div>
        <div v-else-if="permissions.length === 0" class="py-16 text-center text-sm text-slate-400">
          暂无权限数据，先创建一个权限吧。
        </div>
        <div v-else class="overflow-x-auto">
          <table class="min-w-full divide-y divide-white/[0.06]">
            ...
            <tbody class="divide-y divide-white/[0.06]">
              <tr v-for="permission in permissions" :key="permission.id">
                ...
                <td class="px-4 py-4 text-sm text-slate-400">
                  <div class="font-medium text-slate-100">{{ permission.name }}</div>
  ...
  >
    <div class="w-full max-w-2xl rounded-2xl border border-white/10 bg-[#13141b] shadow-2xl">
      <div class="border-b border-white/[0.06] px-6 py-4">
        <h2 class="text-lg font-semibold text-slate-100">{{ roleModalTitle }}</h2>
        <p class="mt-1 text-sm text-slate-400">角色名唯一，权限分配使用启用中的权限码。</p>
      </div>
      <form class="space-y-4 px-6 py-5" @submit.prevent="saveRole">
        <div>
          <label class="mb-1 block text-sm font-medium text-slate-300">角色名</label>
          <input
            v-model="roleForm.name"
            type="text"
            required
            class="w-full rounded-lg border border-slate-700 bg-white/[0.05] px-3 py-2 text-sm text-slate-200 outline-none ring-indigo-500/20 placeholder-slate-500 focus:ring"
            placeholder="例如：content_admin"
          />
        </div>
        <div>
          <label class="mb-1 block text-sm font-medium text-slate-300">角色描述</label>
          <textarea
        ...
        <div>
          <label class="mb-1 block text-sm font-medium text-slate-300">权限列表</label>
          <select
            v-model="roleForm.permissions"
            multiple
            class="h-48 w-full rounded-lg border border-slate-700 bg-white/[0.05] px-3 py-2 text-sm text-slate-200 outline-none ring-indigo-500/20 focus:ring"
          >
            <option
        ...
        <label class="flex items-center gap-2 text-sm text-slate-300">
          <input
            ...
            type="checkbox"
            class="rounded border-slate-700 bg-white/[0.05]"
          />
          保存后保持角色为启用状态
        </label>
        <div class="flex justify-end gap-3 border-t border-white/[0.06] pt-4">
          <button
            type="button"
            class="rounded-lg border border-slate-700 px-4 py-2 text-sm text-slate-300 hover:bg-white/[0.06]"
  ...
  >
    <div class="w-full max-w-2xl rounded-2xl border border-white/10 bg-[#13141b] shadow-2xl">
      <div class="border-b border-white/[0.06] px-6 py-4">
        <h2 class="text-lg font-semibold text-slate-100">{{ permissionModalTitle }}</h2>
        <p class="mt-1 text-sm text-slate-400">停用权限后，会自动从角色权限码中移除。</p>
      </div>
      <form class="space-y-4 px-6 py-5" @submit.prevent="savePermission">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <label class="mb-1 block text-sm font-medium text-slate-300">权限代码</label>
            <input
          ...
          <div>
            <label class="mb-1 block text-sm font-medium text-slate-300">权限名称</label>
            <input
        ...
        <div>
          <label class="mb-1 block text-sm font-medium text-slate-300">权限描述</label>
          <textarea
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <label class="mb-1 block text-sm font-medium text-slate-300">关联资源</label>
            <select
              v-model="permissionForm.resource"
              class="w-full rounded-lg border border-slate-700 bg-white/[0.05] px-3 py-2 text-sm text-slate-200 outline-none ring-indigo-500/20 focus:ring"
            >
          ...
          <div>
            <label class="mb-1 block text-sm font-medium text-slate-300">动作</label>
            <select
...
<script setup lang="ts">
  ...
  const getStatusClass = (isActive: boolean) => {
    return [
      ...
      isActive ? 'bg-emerald-500/15 text-emerald-400' : 'bg-slate-500/15 text-slate-400',
    ];
  };
