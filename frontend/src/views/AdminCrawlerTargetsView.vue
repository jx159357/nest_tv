<template>
  <div class="crawler-targets">
      ...
      >
        <div class="crawler-targets__card-header">
          <div class="crawler-targets__card-info">
            ...
            <span :class="['crawler-targets__status', target.enabled ? 'crawler-targets__status--enabled' : 'crawler-targets__status--disabled']">
              {{ target.enabled ? '已启用' : '已禁用' }}
            </span>
            <span v-if="target.isActive" class="crawler-targets__status crawler-targets__status--active">
              前台展示中
            </span>
          </div>
          <div class="crawler-targets__card-actions">
            ...
            <button
              :class="['crawler-targets__action-btn', target.enabled ? 'crawler-targets__action-btn--disable' : 'crawler-targets__action-btn--enable']"
        ...
        <div class="crawler-targets__card-body">
          <div class="crawler-targets__card-url">
            ...
            <a :href="target.baseUrl" target="_blank" rel="noopener noreferrer">{{ target.baseUrl }}</a>
          </div>
          <p v-if="target.description" class="crawler-targets__card-desc">{{ target.description }}</p>
    ...
    <!-- 创建/编辑对话框 -->
    <div v-if="showCreateDialog || editingTarget" class="crawler-targets__dialog-overlay" @click.self="closeDialog">
      <div class="crawler-targets__dialog">
        <h2 class="crawler-targets__dialog-title">{{ editingTarget ? '编辑数据源' : '添加数据源' }}</h2>
        <form class="crawler-targets__form" @submit.prevent="handleSubmit">
          <div class="crawler-targets__form-group">
            ...
            <input v-model="formData.name" type="text" class="crawler-targets__form-input" required />
          </div>
          <div class="crawler-targets__form-group">
            <label class="crawler-targets__form-label">基础URL *</label>
            <input v-model="formData.baseUrl" type="url" class="crawler-targets__form-input" required />
          </div>
          <div class="crawler-targets__form-group">
            <label class="crawler-targets__form-label">描述</label>
            <textarea v-model="formData.description" class="crawler-targets__form-textarea" rows="2"></textarea>
          </div>
          <div class="crawler-targets__form-row">
            <div class="crawler-targets__form-group">
              <label class="crawler-targets__form-label">优先级</label>
              <input v-model.number="formData.priority" type="number" class="crawler-targets__form-input" min="0" />
            </div>
            <div class="crawler-targets__form-group">
              <label class="crawler-targets__form-label">最大页数</label>
              <input v-model.number="formData.maxPages" type="number" class="crawler-targets__form-input" min="1" />
            </div>
            <div class="crawler-targets__form-group">
              <label class="crawler-targets__form-label">请求间隔(ms)</label>
              <input v-model.number="formData.requestDelay" type="number" class="crawler-targets__form-input" min="500" />
            </div>
          </div>
          <div class="crawler-targets__form-group">
          ...
          <div class="crawler-targets__form-row">
            <div class="crawler-targets__form-group">
              ...
              <input v-model="formData.selectors.title" type="text" class="crawler-targets__form-input" />
            </div>
            <div class="crawler-targets__form-group">
              <label class="crawler-targets__form-label">描述</label>
              <input v-model="formData.selectors.description" type="text" class="crawler-targets__form-input" />
            </div>
          </div>
          <div class="crawler-targets__form-row">
            <div class="crawler-targets__form-group">
              <label class="crawler-targets__form-label">海报</label>
              <input v-model="formData.selectors.poster" type="text" class="crawler-targets__form-input" />
            </div>
            <div class="crawler-targets__form-group">
              <label class="crawler-targets__form-label">评分</label>
              <input v-model="formData.selectors.rating" type="text" class="crawler-targets__form-input" />
            </div>
          </div>
          <div class="crawler-targets__form-row">
            <div class="crawler-targets__form-group">
              <label class="crawler-targets__form-label">导演</label>
              <input v-model="formData.selectors.director" type="text" class="crawler-targets__form-input" />
            </div>
            <div class="crawler-targets__form-group">
              <label class="crawler-targets__form-label">演员</label>
              <input v-model="formData.selectors.actors" type="text" class="crawler-targets__form-input" />
            </div>
          </div>
          <div class="crawler-targets__form-row">
            <div class="crawler-targets__form-group">
              <label class="crawler-targets__form-label">类型</label>
              <input v-model="formData.selectors.genres" type="text" class="crawler-targets__form-input" />
            </div>
            <div class="crawler-targets__form-group">
              <label class="crawler-targets__form-label">上映日期</label>
              <input v-model="formData.selectors.releaseDate" type="text" class="crawler-targets__form-input" />
            </div>
          </div>
          <div class="crawler-targets__form-group">
            <label class="crawler-targets__form-label">下载链接</label>
            <input v-model="formData.selectors.downloadUrls" type="text" class="crawler-targets__form-input" />
          </div>
          </template>
          ...
          <div class="crawler-targets__form-actions">
            <button type="button" class="crawler-targets__form-cancel" @click="closeDialog">取消</button>
...
const listingUrlsText = computed({
  ...
  set: (val: string) => {
    formData.value.listingUrls = val.split('\n').map(u => u.trim()).filter(Boolean);
  },
});
